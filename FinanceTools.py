# FINANCE TOOLS
# Contains 2 functions: first, determine de current annual rate (according to S&P 500 yield)
# Then, determine de amount of final capital expected from annual rate, principal, contributions and compounding 
# periods

# Tags: datetime, functions, csv files, convert str to date, difference between dates, aritmetic operations like addition, 
# subtraction, multiplication, division and power, compound interest, finance, file handling, unittest, error handling, 
# exceptions handling
...
def get_annual_rate():
    ...
    file = pandas.read_csv(r'C:\Users\User\OneDrive\Projects\web scrapping\investment-simulator\SnpPrices.csv')
...
def get_final_capital(principal,contribution,compounding_periods):
    ...
    # Create file
    #with open('FinalCapital.csv','w',newline= '',encoding='UTF8') as f:
    #    writer = csv.writer(f)
    #    writer.writerow(headers)
    #   writer.writerow(data)
    
    # Read csv file already created
    pandas.read_csv(r'C:\Users\User\OneDrive\Projects\web scrapping\investment-simulator\FinalCapital.csv')
    
    # Append new data
    with open('FinalCapital.csv','a+',newline='',encoding='UTF8') as f:
        writer = csv.writer(f)
        writer.writerow(data)
